...
##-----------------------------------------------------
#Data is first organized in different folders for each image (total of 39 folders) with each folder containing 
#specific values for H3K4me3, MyoG and Dapi Sum Intensity - values for each nuclei. 
#1. Step 1: Importing h3k4me3, myog and dapi intensity spreadsheet
#2. Step 2: Extracting two columns from each of the spreadsheets(sum intensity column and nuclear id)
#3. Step 3: Merging H3k4me3, Dapi and Myog intensity spreadsheets 
...
folder_path <- "/Users/therandajashari/Documents/experiments_2024/20240124_comb_seq_comparisons/240124_H3K4Me_bl/2402_spreadsheets/specific_sum_values"
...
write.csv(all_data_grouped, file = "all_sum_data.csv", row.names = FALSE)
write.csv(filtered_data, file = "all_sum_data_filtered.csv", row.names = FALSE)
...
write.csv(average_data, file = "all_sum_average_data.csv", row.names = FALSE)
